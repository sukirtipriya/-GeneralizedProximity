computeDrugDiseaseNetwork <- function(list_disease,list_drug,graph_info,metric){
    ...
    if( nrow(DG_degree_distribution) > 0 ){
      ...
      tmp2 <-lapply(list_drug, function(y){
        ...
        if( nrow(DT_degree_distribution) > 0 ){
          ...
          proximity <- computeProximity(drug_targets,disease_genes,graph_info,metric)
          
          random_distribution_proximity <- computeRandomProximity(DT_degree_distribution,DG_degree_distribution,graph_info,metric)
